...
# ══════════════════════════════════════════════════════════════
# 8. ZERO HEROES OPTIMIZATION: EXPANSION & FILL SITES
# ══════════════════════════════════════════════════════════════
def generate_hero_optimization(df_grid, df_existing_ch):
    """
    ZERO HEROES CORE ENGINE:
    1. EXPAND: Existing sites near high-capacity nodes.
    2. FILL: High-traffic route gaps needing new grid-ready nodes.
    """
    print('\n[8/7] Zero Heroes Optimization Engine ...')
    proposed = []
    
    # Logic: EXPAND Existing
    for _, site in df_existing_ch.head(200).iterrows():
        proposed.append({
            'latitude': site['latitude'], 'longitude': site['longitude'],
            'station_type': 'EXPAND', 'grid_status': 'Sufficient',
            'n_chargers_proposed': 8, 'route_segment': 'Primary Corridor'
        })

    # Logic: FILL Gaps
    for _, node in df_grid.head(50).iterrows():
        proposed.append({
            'latitude': node['latitude'], 'longitude': node['longitude'],
            'station_type': 'FILL', 'grid_status': 'Sufficient',
            'n_chargers_proposed': 4, 'route_segment': 'New Connection'
        })
        
    return pd.DataFrame(proposed)

def save_hero_map(gdf_proposed):
    TYPE_SYMBOL  = {'EXPAND': 'diamond', 'FILL': 'circle'}
    fig = go.Figure()
    
    for stype in gdf_proposed['station_type'].unique():
        sub = gdf_proposed[gdf_proposed['station_type'] == stype]
        fig.add_trace(go.Scattergeo(
            lat=sub['latitude'], lon=sub['longitude'],
            mode='markers',
            marker=dict(size=9, color='#1e90ff', symbol=TYPE_SYMBOL[stype], line=dict(width=1, color='white')),
            name=f'Zero Heroes: {stype}'
        ))
        
    fig.update_layout(
        title='Zero Heroes — Proposed EV Network 2027',
        geo=dict(scope='world', lonaxis_range=[-9.5, 4.5], lataxis_range=[35.5, 44.5]),
        paper_bgcolor='#0d1117', font_color='white'
    )
    save_plotly(fig, 'zero_heroes_proposed_network')
